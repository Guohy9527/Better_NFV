...
#endif
/* HAVE_IBV_DEVICE_COUNTERS_SET_V42 || HAVE_IBV_DEVICE_COUNTERS_SET_V45 */
			 ...
			 struct rte_flow_error *error)
{
	...
	if (flow->actions & MLX5_FLOW_ACTION_COUNT) {
		...
		int err = mlx5_glue->query_counters
			       (flow->handle->counter->cs, counters,
				RTE_DIM(counters),
		...
		qc->hits_set = 1;
		qc->bytes_set = 1;
		qc->hits = counters[0] - flow->handle->counter->hits;
		qc->bytes = counters[1] - flow->handle->counter->bytes;
		if (qc->reset) {
			flow->handle->counter->hits = counters[0];
			flow->handle->counter->bytes = counters[1];
		}
		return 0;
	}
				  ...
				  struct rte_flow_error *error)
{
	...
	if (!flow->handle->counter) {
		flow->handle->counter = flow_verbs_counter_new(dev,
							       count->shared,
							       count->id);
		if (!flow->handle->counter)
...
#elif defined(HAVE_IBV_DEVICE_COUNTERS_SET_V45)
	counter.counters = flow->handle->counter->cs;
	flow_verbs_spec_add(&dev_flow->verbs, &counter, size);
#endif
	return 0;
}
		    ...
		    struct rte_flow_error *error)
{
	...
	return rte_flow_error_set(error, ENOTSUP,
				  ...
				  NULL,
				  "Verbs engine only works with cache mode");
#endif
	if (items == NULL)
		return -1;
...
flow_verbs_destroy(struct rte_eth_dev *dev, struct rte_flow *flow)
{
	...
	if (flow->handle->counter) {
		flow_verbs_counter_release(flow->handle->counter);
